...
连续交互合并成一个 Monaco undo 历史项，移开源码
焦点或按 `Esc` 只关闭临时工具界面，不撤销已经写入的修改。

若后续模型表达式求值失败，Studio 仍保留此前已经成功求值的模型与源码 trace；因此
错误位置之前的 fillet/chamfer 等上下文工具仍可进入并修正源码。若报错的调用本身
就是 fillet/chamfer，工具会以其已成功求值的输入模型为主体，并过滤已经不存在的边
ID，使该调用自己的边和尺寸参数也可以直接修复。
