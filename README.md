...
- 用户手写 `fillet(radius, [])` 或 `chamfer(distance, [])` 后，光标进入空数组即开启 viewport 选边；悬停和选中边分别高亮，确认后只把数字 ID 写回该数组。
...
直接写下空数组可以先保留未修改的输入形状，再在 viewport 中选择边：
...
只有 `Enter`/`Apply` 会替换源码中的空数组。当前交互入口要求第二个参数是直接写在调用中的空数组字面量。
