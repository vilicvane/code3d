...
分栏取消恢复的是本次拖动前的用户代码宽度，而非默认宽度。进入上下布局时保留最后一次
左右布局的文件树/边框宽度，取消仍立即同步 CSS 与 ARIA，不等待下一次 ResizeObserver；
...
工具栏右上对齐，以纯图标分为编辑、绘制、视图三组；名称和操作说明使用原生 hover 提示，
...
不拆散工具组，也不遮住编译提示。操作提示和绘制数字输入同样浮于画布上；
草图显示期间隐藏 3D 面板、gizmo 和源码更新浮层。重编译失败且光标仍在原选中源码
...
有约束时鼠标是软目标；仅在本次拖动关联的连通部分内，若无上游锁定、fixed 或完整
X/Y 定位点，按点声明顺序临时固定第一个非拖动点。关联沿共享端点、圆弧圆心/端点和
显式约束传递，不按坐标重合或几何相交判断；无关锁定点不阻止补锚点。半径手势从圆心
所在部分选取，无关联候选时不从全图回落。该锚点是临时硬锁，不写入源码，也不改变 snapshot 表示的持久自由度。普通运行时
...
临时锚点仍取关联部分内第一个非拖动点，不另加中心固定或整体平移规则。
