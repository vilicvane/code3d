...
（度）；这些是本次创建的数值，不是持久尺寸约束。空字段跟随鼠标，已输入的量
...
预览复用 SVG 与文字节点，不在每次输入中替换文字或重写不可见的 placeholder，
避免打断浏览器的连续输入撤销组。
...
记录可编辑 sourceRef；编程生成的数组仍可显示。Resolver 只接受显式 tuple 和
数字 ID，移动只替换数字坐标，不覆盖表达式；其他 tuple 和注释保持原文。
删除本层点同时移除连接它的本层线，不改变只读上游，也不改写下游源码引用。
