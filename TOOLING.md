...
Select 模式下 Shift 点击增减选择，普通点击替换选择，空白点击或 Esc 清除。选中点后
...
启用并高亮，点击移除所选目标上的该类约束；多选只有部分已有时显示混合态，点击补齐，
全部已有后再点移除。移除时保留可编辑几何的当前显示值，不返回旧求解初值；表达式仍保留。
...
线间平行、垂直、等长等新约束尚未加入；本轮只接入已有约束模型。

单击图中约束标记（或聚焦后 Enter/Space）切回 Select 并选中相关点或整个源码曲线。
本层数值字面量尺寸直接打开输入框并选中完整原值；确认只替换对应约束的值节点，
保留顺序、注释和其他约束，一次撤销。取消、选择或源码变化关闭未提交输入。
表达式驱动的值与只读上游约束可以选择，但不提供数值改写；表达式继续在代码中编辑。
