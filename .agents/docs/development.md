...
定位特定示例时，设置 `CODE3D_EXAMPLE_PATTERN` 为示例相对路径的正则表达式；
过滤后再分片，不运行额外的工程/agent 检查。GitHub CI 的手动触发入口提供同名
含义的 `example_pattern` 输入，填写后仅运行选中的示例，留空仍执行完整 CI。
示例测试记录加载、各源码焦点、编辑和撤销的阶段耗时；源码撤销失败同时输出
实际源码、编辑器焦点、历史可用性和预览版本，避免只有总超时而无法区分原因。
