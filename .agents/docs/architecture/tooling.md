...
| `distance(a, b, axis?)` 调用名 / 空白                 | 两个被测有限元素，同等高亮；距离线和数字 | 两元素所属模型、用于方向的引用模型及调用时关系闭包，全部灰色弱化 | 采用此次调用共同求解的位姿；不扩张到使用测量数值的后续模型；不增加参数面板或选择工具                           |

当前普通拓扑引用会弱化所属模型，单独的命名参考（如 bound）则保留常规模型材质；
两者的高亮几何分别是实际拓扑与参考标记。`distance` 调用名与空白处统一弱化参与模型；内部变量恢复实体焦点，内部元素引用保持所属模型弱化、只突出具体元素，另一被测元素的标记使用 0.7 次级倍率。明确选中 bound 时显示该面的法向标记；其他被测 bound 不显示方向。轴参数中的模型或轴引用同样保留各自焦点。
...
number。调用追踪关联 site / execution，快照包含参与元素、求解位姿、距离和端点。
