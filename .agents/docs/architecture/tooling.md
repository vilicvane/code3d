...
从内向外查找 inspector 时，保留最近可预览调用的默认结果。外层 inspector 可以
接管该值；放行后不能把内层结果替换成外层调用的默认结果。`cut` 工具 inspector
只接管此次调用实际消费的工具；`box(...).relate(...)` 中原 box 与 related 工具是
不同值，不能把前者放进后者的求解位姿表中计算切入体积。
