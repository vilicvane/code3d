...
`@code3d.tool` 显式登记没有数值/拓扑 ID 参数的工具调用；pivotPoint/aroundLine 的点线引用由空间参考选择器编辑，不能误标为接收 edge ID 的参数。
...
记忆变体菜单。工具选择、Alt 和手势状态由 TransformGizmo 持有，显示与拾取消费同一
...
求解结果中的旋转坐标架和现有参数编辑路径，保留 pivot/pivotPoint/pivotVertex/aroundEdge/aroundLine、上游表达式与调用顺序。共享指针所有者按实际 binding
...
位置拖动始终网格吸附，包括按住 Alt 切换到参考平移后开始的手势。XYZ 距离文本框
按当前网格小格调整 step，显式数字输入保持精确值；旋转
...
Core 快照的 relationStages 是源码工具定位的分段依据，transformations 保留各次动作的 sourceRef 和结果架。选择多约束段的 self 时展示该段完整结果，查找同段后面最近的独立 offset/rotate；不存在则在该段末尾插入，不能跨到下一个约束段。选中某个独立变换时，同工具修改当前调用，另一工具在当前数组项之后插入新的独立调用；单值 return 改为数组。完成后的独立变换不允许接链。显式约束链仍按其联合求解能力限制刚体预览，不把任意一条约束当作整段变换。

relate 直接返回数组内的空白也是 self 源码目标，保留实际 callback 的执行实例；按邻近关系所属阶段展示联合解，空数组也提供独立变换入口。数组/return 插入位置由当前源码 AST 确认，与 Core 构造函数导入共同组成一次源码事务；复用已有别名或 namespace，新增名称避开已有绑定。源码无法安全定位为返回值或数组项时不虚构插入位置。新增操作的焦点以替换文本内偏移记录在源码事务中，编辑器合并导入变化计算最终位置；同一次 Undo 恢复源码和光标。共享 map 的每次执行携带自己的 owner 和动作架，预览按各实例计算，提交仍只改一处源码，取消不改文件，Undo 同时恢复调用和导入。
