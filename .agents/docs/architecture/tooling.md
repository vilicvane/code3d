...
“相关”按正在检查的操作和运行实例选择，不等同于整个文件、完整依赖图或最终导出。
先决定实体范围和阶段位姿，再决定模型明暗与元素标记；这三个判断不能互相替代。
函数与方法通过同一 operation 输入角色归一化，数组成员仍保留单独的来源。
只有参与几何组合的输入角色互相补充同伴；`source` 与 `reference` 表示派生来源或
命名引用，不因出现在同一调用就生成额外模型。`relate` 的参考上下文由关系规则单独决定。

从内向外查找 inspector 时，保留最近作用域的默认预览：调用名与实参采用调用结果，
receiver 与已放行的 closure body 保留关注值。外层 inspector 可以接管该值；放行后
不能用外层调用结果替换已有预览。`cut` 工具 inspector 要求全部可预览关注值都是此次
实际消费的工具；不能只筛选实体而吞掉其他关注值。`box(...).relate(...)` 中原 box 与
related 工具是不同值，不能把前者放进后者的求解位姿表中计算切入体积。

| 源码关注位置                                          | 前景 / 参与元素                      | 补充的相关实体                                                       | 位姿与装饰规则                                                                                                 |
| ----------------------------------------------------- | ------------------------------------ | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| 独立模型变量、普通构造结果                            | 当前值；集合为其成员                 | 不因后续使用自动加入所有消费者                                       | 单个值显示局部坐标；集合显示组合位置                                                                           |
| 直接内联于组合操作的构造器                            | 当前构造结果                         | 外层同次组合操作的其他输入                                           | 保留当前构造器的参数工具，使用组合位置；独立声明不隐式扩张                                                     |
| `group`、`union`、`cut`、`intersect` 的调用 / 结果    | 组合结果                             | 结果本身包含的几何，不叠加全部旧输入                                 | 结果的局部坐标；查看输入时才展开输入上下文                                                                     |
| 上述组合操作的 receiver / 单个参数 / 数组成员         | 当前输入；数组空白为整个集合         | 同次调用的其他输入，按 node 去重                                     | 输入使用组合位置；同伴灰色弱化，切削工具半透明；Boolean 输入另画交集或截线；`intersect` 显示所有输入的共同结果 |
| `originOffset`、原点调整、旋转、缩放                  | 当前步骤的模型                       | 沿值的来源链找到后续组合消费者的同伴                                 | 保留当前步骤，排除同来源的最终版本；后续变换使 Boolean 区域不再对应当前几何时不画该区域                        |
| 完整 `relate(...)` 调用（参数外）                     | 当前 relate 输出                     | 本次 relate 引用的模型为次级；后续组合同伴为外围                     | 仅本次新增约束和引用决定主次，不把继承引用都升为次级                                                           |
| `on` / `align` 及其元素参数                           | 当前关注侧的模型和元素               | 同一约束另一侧为次级，组合同伴为外围                                 | 同段约束联合求解；选中阶段不含后续独立变换；只画当前约束两侧的标记                                             |
| relate 内的 `offset` / `rotate` / pivot / axis 选择器 | relate self                          | 被引用参考及后续组合同伴                                             | 当前阶段的原点或旋转参考；该阶段的关系引用保留次级模型强调，但不叠加约束标记                                   |
| 顶点、边、真实面等拓扑引用                            | 所选拓扑                             | 元素所属模型弱化；继承外层操作上下文时沿用外层参与者                 | 使用真实几何高亮                                                                                               |
| 命名点、轴、bound 引用                                | 参考标记                             | 单独查看时保留所属模型常规材质；外层关系 / 操作决定其上下文层级      | bound 使用有限参考面和角线，轴 / 面标记保留方向语义                                                            |
| `fillet` / `chamfer` 结果                             | 修改后模型                           | 额外叠加原输入的淡化几何及所选原边                                   | 输入与结果使用 operation 记录的相对变换，便于比较                                                              |
| `shell` 等拓扑参数选择                                | 操作输入中的候选与所选拓扑           | 所属输入模型                                                         | 由 topology-selection 作用域提供候选，不套用 fillet 的原边比较叠层                                             |
| `loft` 的截面输入                                     | 当前截面                             | 同次调用的其他截面和淡化结果                                         | 结果固定在第一截面坐标架；参与者采用组合位姿                                                                   |
| `extrude` 的单个 / 数组输入                           | 当前输入及其拉伸结果                 | 同次调用的其他输入和弱化结果                                         | 每个结果固定在自己的输入坐标架；跳过与当前步骤不匹配的后续结果                                                 |
| `wrap` 的轮廓 / 目标面                                | 当前关注输入                         | 同次其他输入和结果                                                   | 有限面引用与轮廓共用调用时求解坐标架                                                                           |
| `thicken` 的面 / 厚度                                 | 当前源面                             | 增厚结果                                                             | 每个结果继承对应源面摆放，厚度经普通 length 参数编辑                                                           |
| `expose` 调用 / receiver                              | 已有模型及其组内成员                 | 不增加被命名引用的独立副本                                           | 只扩充元素名称；选择映射中的引用仍可单独检查该引用                                                             |
| `distance(a, b, axis?)` 调用名 / 空白                 | 两个被测对象，同等强调；距离线和数字 | 元素所属模型、方向引用模型及关系闭包灰色弱化；整实体输入保留模型外观 | 采用此次调用共同求解的位姿；不扩张到使用测量数值的后续模型；不增加参数面板或选择工具                           |

当前普通拓扑引用会弱化所属模型，单独的命名参考（如 bound）则保留常规模型材质；
两者的高亮几何分别是实际拓扑与参考标记。`distance` 调用名与空白处同等强调两个被测对象；整实体或 group 输入直接使用已有模型外观，不叠加整个实体的青色面填充。内部变量恢复该实体焦点，另一实体及关系上下文灰色弱化；内部元素引用保持所属模型弱化、只突出具体元素，另一被测元素的标记使用 0.7 次级倍率。明确选中 bound 时显示该面的法向标记；其他被测 bound 不显示方向。轴参数中的模型或轴引用同样保留各自焦点。
...
`distance` 通过 `beginModelEvaluation` 的作用域观察器记录不可变快照；普通 API 仍返回
number。调用追踪关联 site / execution，快照包含参与元素、整模型输入标识、求解位姿、距离和端点。
普通测距复用内核缓存的最短距离端点；沿轴测距线连接投影区间边界，端点是参考位置，
横向优先取两侧投影 bounds 的重叠范围中点，因此位于面投影内的点会直接成为线端；横向不重叠时取最近边界中点。一般几何的参考端点不冒充实体上的最近点；测量轴投影重叠显示 0。视口的 sourceContext 和既有装饰 reaction
派生显示，切换调用清理旧装饰与标签纹理；不再求解、按返回数字查找或保存第二份 UI 状态。
标注使用灰阶虚线与两端短刻线，中性数字旁的 X/Y/Z 字母复用视口轴色。
虚线节距、刻线和文字保持屏幕尺寸；仅明确选中的 bound 显示法向箭头。测量上下文附着于调用内部的原有源码目标，不覆盖参数的选中身份；实参追踪保留拓扑与命名引用，成员访问另行记录实际 receiver，`a.right` 中的 `a` 与 `right` 因而分别指向实体和 bound；不重复求值 getter，也不截断 optional chain。别名和同一实体上的不同元素均能识别。循环中在调用与参数之间导航保留同一测量快照实例。
实时视图和 PNG 导出走同一绘制路径，装饰不参与 CAD 导出与拾取。

实现入口：[operation-context](../../../packages/app/src/model/operation-context.ts)、
[executor](../../../packages/app/src/model/executor.ts)、
[operation-decorations](../../../packages/app/src/model/operation-decorations.ts)、
[measurement-decorations](../../../packages/app/src/model/measurement-decorations.ts)。
测量验证：[源码上下文](../../../packages/app/test/distance-focus.test.ts)、
[真实绘制与 PNG](../../../packages/app/test/browser/distance.test.ts)。
...
Box 尺寸参数通过通用 inspector 返回 `style: 'edge'` 的 Dimension：对应实体边使用
亮绿色实线，保留轴名和实际长度；普通测距仍使用灰阶虚线与端部刻线。省略参数的尺寸
取实际返回几何，不从未提供的实参读取。Inspect 记录空括号和末尾逗号后的下一个参数
槽位，使用当前调用求值的实参数量（包括 spread）匹配形参；空槽位没有 focused 值，
不能将调用返回模型误当成该参数的焦点。这些信息沿现有检查快照和响应式视口消费。
...
原点拖动固定手势开始的 snapshot，旋转使用新旧完整旋转的差。模型 originOffset 工具的
Alt 待机手柄移到对象当前世界包围盒中心，轴向不变；松开 Alt 后手柄回到原点。
Alt 手势固定为移动对象：源码偏移取手柄位移的反向，预览几何随手柄移动、原点标记
保持在原位；普通手势保持几何不动并移动候选原点。手势开始后切换 Alt 不改变该手势。
空间工具栏位于参数面板上方。模型表达式提供 originOffset、模型 rotate；同一流式
调用链先匹配当前步骤之后的同类调用，再向前匹配，无匹配才以当前表达式为锚点新增。
relate 工具区分平移、绕点旋转、绕轴旋转；两种旋转复用草图的
...
固定操作，不影响吸附；relate 平移工具不响应 Alt。SpatialToolbar 持有短期参考选择会话，
...
项目位置菜单统一提供打开/切换本地目录、复制到本地、重连与返回浏览器存储等入口；
头部工具栏只保留文件创建、目录创建与刷新。菜单命令沿用位置操作的 observable busy 状态与原生目录选择流程。
...
全局相机记录。交互预览不重置视角；新模型求值和 Inspect 场景接纳后，几何投影
超出屏幕安全区时，保持朝向和投影平滑退远，尺寸缩小不自动拉近。光标/参数高亮
变化不触发取景；导航、相机过渡、工具拖动和临时预览期间暂缓，结束后检查最新结果。
每个场景的相机记忆同时保存保留局部视图的意图：仅在用户缩放或平移结束后，按实际
几何的屏幕投影更新，边界使用 3px 滞回；主动裁切时暂停自动取景，主动退回全景或
Fit/重置视图后恢复。模型更新、旋转和程序化相机变化不重判用户意图。
由 [ViewportViewState](../../../packages/app/src/model/viewport-view-state.ts) 统一持有
各场景的相机记录、局部意图、已接纳求值和已检查求值；求值身份使用弱映射生成轻量
标记，历史相机不保留旧模型快照。待检查状态从两个求值标记派生，导航/过渡/工具拖动
与当前呈现类型共同决定检查时机。同一求值的焦点更新不重复排队，新求值不以包围盒
相等作为几何变化判据；检查也消费局部模式的当前结果，之后仅改变意图不重复检查。
同场景更新不恢复相机目标；坐标架确实变化时一并转换当前姿态、手势起点和过渡端点，
保留过渡进度。临时预览只呈现临时画面，不接纳正式场景或覆盖其相机/检查记录，恢复时
回到已保存的正式场景。以上通过 MobX 派生联动，不轮询原生相机字段。
新的显示范围按对应规则恢复或初始化。相机与导航细节由
...
3D 视口按变化绘制：模型与装饰的原生资源更新、导航和手柄事件、尺寸与像素比变化、
异步纹理加载共同请求下一帧，同一帧内的请求合并。静止场景不持续提交 WebGL 绘制，
避免软件渲染占用编译和交互所需的 CPU。相机过渡期间连续绘制，过渡结束即停止；
导出沿用相同渲染管线并恢复屏幕装饰尺寸。调度归属 ModelViewport，不复制工具或
预览状态，页面结束时取消待绘制帧并释放导航、手柄订阅和尺寸观察。
