...
[Browser projects](../../../packages/app/src/project/browser-projects.ts)管理浏览器项目
清单、名称与创建/删除；每个文件系统实例只管理单个项目。既有浏览器文件继续属于
`Default project`，新增项目使用独立 IndexedDB、工作区身份、构建缓存和 agent 身份。
每个项目都有自己的 URL，多个 tab 可分别打开不同项目。位置菜单在 Browser projects
标签下列出浏览器项目，以既有选中项样式高亮当前项目。点击任一项目名称切换项目；
每行的尾部箭头、整行悬停或项目名称上的 ArrowRight 展开该项目的二级菜单：
首项 Open 复用点击名称的保存后切换流程，其后为 Copy to local folder and open，
...
首次使用的默认浏览器项目自动提供默认模型和示例。浏览器项目清单记录可选的
`template: 'examples'`，表示新建时选择示例模板或显式重置后的默认内容；缺省表示空项目。初始化仅在项目为空且
示例目录尚无托管状态时使用这一选择，勾选后生成默认模型和托管示例，未勾选则不创建
`model.ts` 或 `/examples`。主页面和后台操作共用 `initializeBrowserProjectContents`，
以唯一入口执行 seed。主页面另行同步已托管示例，后台复制保留已有源文件内容。
页面挂载和初始化通过按项目身份获取的 Web Lock 串行执行；
后台操作持目标项目独占锁，与已打开页面及新的打开请求互斥。
单项目清单以可选 `initializingSeed: true` 记录未完成的种子写入，
格式仍为 `version: 1`，仅在示例和默认模型全部完成后清除此标记。期间刷新会重试未完成的
种子写入，不把部分文件误判为非空用户项目，后打开的 tab 也不会重复覆盖已初始化项目。
首次打开的空本地目录仍通过 Create examples 确认是否添加
示例。单项目清单记录示例的托管状态或跳过决定，重新打开不再询问；之后仍可通过菜单
显式创建示例。`.code3d` 元数据不算作者内容，其他条目均使目录非空。
非空未托管项目不询问、不自动添加示例；未托管的用户
examples 不参与版本同步。显式创建或重置后才纳入托管，已托管目录继续自动更新且
不再次询问。切换本地目录与浏览器项目以当前页面的项目连接为单位。
浏览器项目行的二级菜单支持显式复制到空本地目录并打开，
按字节复制项目文件、配置、资源、示例及空目录，递归排除 `node_modules`、`.code3d` 和
`code3d-lock.json`。复制当前项目与保存共用项目操作队列；复制其他项目前也先保存当前项目。
完成后核对当前项目 revision，期间新增编辑时保留当前页面，不自动离开。
复制失败保留原项目并提示目标可能包含部分文件；不覆盖非空目标。

浏览器项目的 Delete 与 Reset 通过共享确认对话框展示目标项目名称，
明确丢弃目标文件和已安装依赖；操作当前项目时还明确丢弃其未保存编辑。
两种操作均持目标项目独占锁，等待使用该项目的其他 tab 关闭。
删除先清理目标项目构建缓存及 agent 授权、session、回执，再删除项目文件 IndexedDB，
最后移除清单条目。删除最后一个项目后创建身份全新的空 `Default project`，不弹出示例确认。
Reset 保留身份与名称，先持久记录 `template: 'examples'`，再删除其 IndexedDB，
挂载后恢复默认模型与托管示例，不重复询问是否创建示例。
即使后台重建中断，之后打开项目仍能继续初始化。

确认命令及目标身份暂存于当前 tab 的 sessionStorage，不放进可分享 URL。
操作当前项目时重新加载页面，先结束旧编辑、安装和编译任务，再执行破坏性操作；
删除完成后打开另一浏览器项目，重置完成后重新打开该项目。
操作其他项目时直接在当前页面执行，保留当前项目及未保存编辑。
命令在目标数据操作、初始化和构建缓存清理完成后消费；页面退出时保留待执行命令，
等待中刷新仍继续操作原目标，再恢复 URL 指定的原工作区，完成后普通刷新不重复执行。
仅针对当前项目的破坏性命令豁免该项目的未保存提醒，后台命令不豁免当前草稿。
[后台项目操作](../../../packages/app/src/project/browser-project-operation.ts)用一次性 Worker
挂载目标文件系统，执行重建或复制；主页面的 ZenFS 挂载不变。目标独占锁保持到 Worker
结束，完成、失败或页面退出均终止 Worker，释放其数据库连接。
等待期间显示关闭其他使用目标项目的 tab 的提示，不依赖 IndexedDB blocked 事件。
清理失败提示错误并保留清单条目，用户可重新发起操作。
删除与重置不清理其他浏览器项目、本地目录句柄、App 设置或共享几何/下载缓存。
...
默认值并清除草稿。单个数值控件统一拥有草稿、校验、滑条同步及最后接受值，
正常回填与 Reset 共用写入路径；面板分别订阅字段变化和源码关注，高亮切换不触发
字段回填，新增字段自行应用当前关注，两个订阅不依赖执行顺序。参数不写回源码或持久化。
...
`AnimationControls` 拥有播放栏 DOM、细粒度订阅和按钮/页面隐藏事件，销毁时统一
取消订阅、移除监听并停止播放；主 App 只连接可见性与执行条件。播放图标、可用性
和时间读数分别订阅实际显示值，不维护另一份显示时钟。状态栏在
