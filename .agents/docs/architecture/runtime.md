...
Google 字体按规范化 CSS URL（字体名称、字重、斜体）保存完整资源组，包含原 CSS
及其全部解码后的字体子集，Unicode 范围和优先级仍由 Core 的唯一 CSS 解析器解释。
资源组以一条记录整体写入、校验和淘汰，全部下载成功后才发布；复用相同的内存
LRU 和 OPFS 预算，不依赖个别 HTTP 记录的存活或新鲜度。任何响应为 no-store 时
不保留该组。完整命中直接恢复同步资源表，不再经过 CSS HTTP 请求。

文件树空白处的 **Refresh fonts** 通过已有的 MobX 编译命令发送字体刷新选项，
从当前入口重建依赖并绕过该次 Google 字体组、CSS 和字体文件的缓存；普通项目
刷新与 **Clear build cache** 保留完整字体组。刷新失败保留之前完整组并报告实际
错误，不把部分新资源发布为成功解析。刷新请求在准备资源时按请求身份消费，
异步准备期间新到达的刷新不会被旧构建清除。Worker 内资源缓存与跨进程取消沿用
原有生命周期，不为缓存引入 UI 镜像状态。
