...
`boolean-model.ts` 解析自由函数的首操作数；实体校验与布尔求值统一归运行时的组合入口。
三种布尔实例方法接受单实体或非空数组，只在入口归一为列表；自由函数保留数组契约。
两类调用共享操作输入角色、原点、材质和拓扑语义，各 API 的检查器复用完整操作数的
检查逻辑处理方法 receiver 与参数，不引入另一份响应式模型状态。
...
覆盖指定键，旧值不变。普通复制与几何派生保留主输入的元数据，不合并其他输入；
新 group 自有元数据为空，成员仍保留各自的数据。Core 不解释不透明值，不维护元数据
时间线或变换回调，也不把它们放入渲染快照和 Worker 协议。业务包负责数据含义，
几何引用通过 expose 跟随坐标变换。Gears 保存齿数、螺旋角及模数/齿宽比例，
用名义齿宽定义精确轴线，再从当前轴长解析缩放后的尺寸，避免包围盒容差影响模数。
...
`assembleGears` 的外层条目表示轴位：单齿轮或 `[incoming, outgoing]` Tuple。
组装函数将 incoming 与上一轴的 outgoing 啮合，再将同轴 outgoing 的 frame 对齐到
已摆放的 incoming。作者通过 Origin 定义同轴两齿轮的层间偏移；相邻轴以实际齿中心对齐，
不假设模型原点就是齿中心。返回值和参数检查均按顺序展开齿轮，内部刚性连接不占用 pairs 配置。

原点由构造器确定；派生操作继承主输入坐标系，不按结果包围盒自动居中。
group 默认与 union/intersect 一样使用首个成员或操作数的完整局部坐标系，cut 使用 stock，
loft 使用第一截面，extrude、revolve 与 sweep 保留输入面。group 将求解位姿统一左乘首成员位姿的逆，
保留成员相对装配；空 group 为默认坐标系。原点操作显式重表达坐标且不改变旧值。
`frame()` 返回独立、可 relate 的非几何 Frame，直接作为 align 的 frame 引用。
`group(parts, {frame, name})` 可显式采用参考架已求解的完整位姿，不依赖成员顺序。
