...
tuple，辅助曲线使用 `aux:line`、`aux:circle`、`aux:arc` 类型名，几何当前数据与 `constraints` 分开。点、线、圆、圆弧具有本层唯一的正整数
...
同一会话按入口文件、设计上下文和草图执行身份记忆中心与缩放，源码数值修改保留
视图，不把同一调用的多次执行或不同空间值混用。项目重置时清空这些记录。
[SketchNavigation](../../../packages/app/src/ui/sketch-navigation.ts)以 MobX 管理当前
显示姿态，SVG 绘制、拾取与格距读取同一状态。可见草图之间切换时用 300ms
平滑中心与对数缩放；连续切换从已显示的姿态重定向，并记住各自尚未完成的目标。
滚轮、平移或几何交互立即接管显示姿态。隐藏时停止动画；从空视图或 3D 进入草图
直接取景或恢复记忆，反向进入 3D 同样不播放切换动画。减少动态效果偏好直接
应用目标姿态。绘制组件销毁时释放订阅与动画帧。
网格与吸附共用当前缩放对应的格距；图例直接观察当前导航状态，与 3D 共用左下角布局，
退出草图时恢复当前 3D 的读数和可见性。图例及提示布局见
[源码与交互工具](tooling.md#意图计划与事务)，缩放及视图切换由
[导航回归](../../../packages/app/test/browser/sketch-navigation.test.ts)和
[网格回归](../../../packages/app/test/browser/camera-navigation.test.ts)验证。

绘制会话拥有阶段、草稿、吸附候选和数字字段，preview 不写源码。成功绘制一个
图元或一个矩形时，通过[源码事务](tooling.md#意图计划与事务)一起写入几何和约束。
连续线按每段提交和撤销；工具退出或取消不留下半成品。

绘制工具和数字草稿使用 MobX 发布当前步骤，SVG 预览直接消费这些状态。每次确认
保存临时工具检查点：未提交的点击由会话逐步撤销，已提交的图元先由 Monaco 撤销，
再按其原生历史位置恢复工具。重做同样恢复确认后的步骤；格式化和求解初值同步
仍属于对应的源码事务。控制器同时恢复该步骤的临时几何快照，普通编译再发布最新
求解结果；期间下一次绘制不必等待编译，也不会使用已撤销的点 ID 或吸附候选。
快照只恢复显示与交互，不代替 Monaco 写回源码。撤销连续线恢复上一端点；
撤销圆弧恢复圆心、起点、方向和数字输入。取消、切换工具或草图、普通源码编辑
清空检查点，不跨会话复活工具。

吸附目标从当前已提交几何派生并缓存，不随 pointer move 重新求交。实际点
（含端点和圆心）优先并保留引用；再按屏幕距离选择有限线段、圆、有限圆弧的交点、
原图元中点及圆的四个象限点，最后尝试原点、方向和网格。计算位置只写坐标，不增加
持久约束。拖动排除目标点、别名及依赖它的曲线，避免吸附自己的旧几何。显式输入、
轴锁与 Alt 临时关闭共同约束所有候选；三维背景始终只作参考。
...
GUI 修改或添加尺寸、几何约束后，成功求解自动同步本层安全可写的几何初值；
约束变更与初值同步共用一步撤销和重做。同步复用编译诊断提供的安全 Fix，
不在尺寸标记或约束面板中另外推导求解结果。操作记录开始时的几何；普通编译
得到新约束值后，若原有点在线、圆或有限弧上的关系丢失，由项目 runtime 以该
参考几何重求解。连接和有限区间与拖动共用计算，作为本次编辑条件，不追加作者
约束。安全可写坐标经普通源码正向求值验证后写入同一源码事务；表达式参数和
上游保持只读，不能维持连接时保留错误与最后成功视图，不能发布脱离的结果。
待同步操作绑定源码修订与撤销组；同组格式化延续该操作，其他编辑或项目切换
使其失效。编译结果发布前完成安全同步，过渡和失败时保持几何与约束快照一致。
连接修复的异步结果再次校验操作身份及源码版本；撤销、其他编辑或格式化触发的
新编译使旧结果失效。同源码多个独立几何实例仍不能选择其中一个写回。
...
辅助曲线由源码类型名的 `aux:` 前缀表达；快照归一为几何 kind 和 construction 标记。
关闭时只移除前缀，不留 options 或 false。
Select 工具栏按 observable 选集和当前层快照派生 Construction 的可用及混合状态，
一次源码事务切换选中作者曲线；显示分段按 ID 去重，不拆出另一份几何。SVG 与三维视图以固定像素虚线
显示，本层与上游的吸附、约束和点身份照常参与交互；上游及表达式控制的类型名不可
由 GUI 覆盖。控制器在源码提交后同步发布临时快照，撤销仍由源码历史驱动。
...
所有存活片段保留原曲线的 construction 属性，源码复制与即时预览保持一致。
...
普通闭合边界提取区域，实体/孔/岛按包含关系交替。各层 construction 曲线在
边界校验前统一排除，不会生成孔、岛、分叉或相交错误；不从求解器和吸附候选中删除。
普通曲线按解析交点划分有向边，沿切线及曲率顺序遍历有界区域，开放线尾和桥不阻止成面。
相交可以形成多个区域；同顶点回路拆开后按面积和包含关系保留孔与岛。
重叠边界明确诊断；不隐式补线、改作者数据或改 ID。
区域预览不创建作者实体或拦截拾取。
...
草图顶部状态按当前 execution 及上游定义判断错误，不把失败的下游派生、
其他模型或同源码的另一实例混入。viewport 诊断条只保留显式声明的源码与求解结果不同步提示及 Fix，
普通求解错误保留在顶部状态和编辑器。编辑器仍显示完整源码诊断。重新编译失败时
可保留同一源码目标最后成功的只读快照；离开范围清除，成功编译恢复正常编辑。

直接修改源码后，成功求解但几何初值与显示结果超出局部精度时，产生非阻塞
warning，状态仍为 Ready，由用户显式 Fix。GUI 约束编辑的自动同步仅消除本次
操作可安全修复的差异；无法安全同步时保留诊断。通用 diagnostic actions 携带
标签和 ToolIntent，UI 不实现修复算法。自动同步与显式 Fix 都只同步本层可写的
字面量坐标和半径，保留表达式、实体 ID 与约束。
