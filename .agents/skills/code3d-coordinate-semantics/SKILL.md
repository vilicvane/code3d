...
relate 中连续的约束共同求解位姿；初段未约束自由度采用求解器默认结果，后段继承前段姿态。约束链的 `offset` 在原有解的前提下，沿目标参考架平移 self；不得额外固定切向位置或隐式居中。`offset(0, 0, 0)` 不改变解。旋转同样作用于该链原有解，零角度不新增朝向约束；pivot 属于 self 局部坐标，外部 axis 使用其已求解的组合位姿。`offset` 与 `rotate` 严格按调用顺序执行，不跨旋转合并位移。目标架属于当前操作阶段；局部动作与外部轴动作各按所属参考系组合。其他关系仍约束最终结果；独立 Transformation 作用于同段联合解，允许离开此前接触位置。独立变换按数组顺序执行；随后出现约束时进入新段，不把此前约束跨段收集回去。连续 relate 调用接续同一排列。
