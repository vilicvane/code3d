...
- relate 的主次由当前源码关注侧决定：on/align 方法名强调 self；目标括号内的空白、注释及子表达式强调目标元素。显式左侧元素引用强调 source。独立 offset/rotate 及 pivot/axis 选择器的上下文始终是 relate self；约束不支持链式变换。反向写法 `base.on(self.up)` 的目标括号内强调 self。
...
- source 与 target 的头部尺寸、形状一致；不以空心、放大或颜色变化表示主次。关系标记仅从当前选中的 align/on 关系生成；同段其他约束仍共同求解但不叠加轴面高亮。裸 self、offset、pivot/pivotPoint/pivotVertex/axisEdge/axisLine/rotate 不显示无关关系标记，保留当前工具原点或旋转参考。命名元素及普通拓扑引用路径不再叠加一套。
...
- `expose` 的命名引用不添加独立模型或组合同伴；查看调用及其 receiver 只显示已有模型。源码上下文按实际组合输入角色扩张，不能把同次调用的所有输入都当作参与组合的几何。
