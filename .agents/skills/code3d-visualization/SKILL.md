...
- 约束链显式调用显示的姿态、参考几何和工具必须来自同一个当前阶段快照；不能混入光标之后的 offset/rotate 结果。选择裸 self 时显示所属链的完整结果，工具修改后面最近的对应操作；其坐标架和标记需包含后续变换的作用。
