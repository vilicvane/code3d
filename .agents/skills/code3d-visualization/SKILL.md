...
- relate 的主次由当前源码关注侧决定：on/align 方法名及后续 offset/pivot/pivotPoint/pivotVertex/aroundEdge/aroundLine/rotate 等链操作（包含其参数）强调 self；on/align 目标括号内的空白、注释及子表达式强调目标元素。显式左侧元素引用强调 source。反向写法 `base.on(self.up).offset(...)` 的链操作与目标括号内均强调 self。
...
- source 与 target 的头部尺寸、形状一致；不以空心、放大或颜色变化表示主次。关系标记仅从当前选中的 align/on 关系生成；同段其他约束仍共同求解但不叠加轴面高亮。裸 self、offset、pivot/pivotPoint/pivotVertex/aroundEdge/aroundLine/rotate 不显示无关关系标记，保留当前工具原点或旋转参考。命名元素及普通拓扑引用路径不再叠加一套。
- 标记关注侧与引用值的归属分别保留；例如在 `aroundLine(axis.axis)` 内强调 self 时，成员补全仍作用于 axis。未编译的成员候选预览不能混用旧关系快照；编译完成后恢复完整关系预览。
- 面箭头沿面法向；方向元数据与 offset/旋转参考坐标架保持各自含义。
- 约束链显式调用显示的姿态、参考几何和工具必须来自同一个当前阶段快照；不能混入光标之后的 offset/rotate 结果。仅选择裸 self 时显示所属链的完整结果；点击工具后，已有对应操作立即激活其完整表达式末尾和阶段，没有对应操作则显示实际插入位置之前的阶段。坐标架、候选和标记随该阶段同步，不能等首次拖动才切换。
...
- 关系阶段预览保留同一连续约束段的兄弟约束，联合求解；只把当前链替换为选中阶段，不能把各条关系拆成独立位置。独立 Transformation 截止当前操作，同一源码的循环实例各自使用对应阶段快照，不能只更新焦点实例而让其他实例混入后续操作。后续约束属于新段；选择 self 时展示所属段的完整变换结果。当前关系的高亮范围不改变该段求解范围。
...
- 平移工具显示当前阶段的零件原点，随几何的平移预览移动；旋转工具显示对应 pivot/axis。标记由当前工具派生，不能只按源码邻近的旋转选择参考。
...
- 空间工具拖动始终遵循网格或角度吸附。工具栏区分平移、绕点旋转和绕轴旋转，两种旋转在变体菜单中选择；各旋转工具的 Alt 临时切换到对应参考的平移，手势开始后固定操作。平移工具不响应 Alt。旋转工具自动显示对应点/轴候选，点选不依赖 Alt；Alt 只切换对象旋转与参考平移 gizmo，松开后仍保留候选。手柄拾取优先。选择器和 rotate 共用工具及参数面板。 绕点与绕轴属于不同工具：激活时按相邻项规则决定复用或插入，参考编辑写入已选同类调用，插入时创建新操作并保留已有旋转；单纯切换工具不改源码。未填完的 pivotVertex、pivotPoint、pivot、aroundEdge、aroundLine 及其偏移链仍保留当前 relate self 与已完成前置阶段的参考选择，不能借用上一条旋转绑定。选参考时同一事务补齐缺少的 rotate（点为三个零角度，轴为一个），已有旋转角度和偏移保留，重新求值后进入完整旋转 gizmo；未交互不改源码。未完成选择器也立即展示参考、已有偏移和旋转角度的完整面板，缺少 rotate 时显示零角度，实际编辑角度才补写调用。源码提交后，能准确预览且源码锚点有效的既有操作更新参数与坐标基准，保留连续编辑；新增调用、缺少有效写入目标或准确预览时等待新模型。不能只以编译中为理由隐藏，也不能混用同一 occurrence 上不同操作的快照。
