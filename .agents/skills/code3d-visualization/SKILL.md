...
- `distance` 预览保留调用时的参与元素和共同求解位姿，调用名与空白处将模型作为灰色弱化背景，两个被测元素同等高亮。内部参数保留普通源码焦点：变量突出实体，元素引用突出该元素，另一被测侧标记使用与关系相同的次级不透明度倍率；同属一个实体的不同元素仍分别判定。沿用真实面、有限边、点和 bound 的现有样式；测量 bound 默认只保留参考面与角线，明确选中某个 bound 时仅恢复该 bound 的法向标记。
