...
- relate 的主次由当前源码关注侧决定：on/align 方法名强调 self；目标括号内的空白、注释及子表达式强调目标元素。显式左侧元素引用强调 source。独立 offset/rotate 及 pivot/around 选择器的上下文始终是 relate self；约束不支持链式变换。反向写法 `base.on(self.up)` 的目标括号内强调 self。
