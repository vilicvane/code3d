...
- 普通选择框使用快照中方向 bound 的精确局部几何范围，并应用当前实例变换，与六个参考面保持重合；不从渲染网格或旋转后的子网格包围盒重新估算世界轴范围。
...
- 参数边高亮可保留轴名与长度数字，但对应边采用亮绿色 `#d8ff3e` 实线，不用灰阶测距虚线或端部刻线。样式由 inspector 的标注声明，renderer 不按函数名判断；空参数位置与已填参数保持相同高亮语义，数字使用模型实际采用的尺寸。
...
- 新模型求值与 Inspect 场景接纳后统一检查实际几何投影；包围盒相等不能代表几何不变。超出屏幕安全区才保持朝向和投影平滑退远，变小不自动拉近。以每场景已接纳/已检查求值派生待取景状态；光标或参数高亮切换不重复检查，也不恢复相机目标。导航、工具拖动与临时预览结束后再检查最新结果。
- 同场景的坐标架变化同步转换当前相机、手势起点与过渡端点，保持动画进度。临时预览不改正式场景记忆和检查进度；还原预览与接纳新求值使用各自明确的入口。
- 用户缩放/平移结束时，按实际几何投影是否完整可见记录保留局部视图的意图，与场景相机一起记忆；边界使用像素滞回。模型更新、自动取景与单纯旋转不推断新意图。主动退回全景或 Fit/重置后恢复自动取景。
- 验证透视/正交、真实滚轮和平移、参数面板编辑、场景切换、动画与交互中断；辅助标记不参与模型是否完整可见的判断。
