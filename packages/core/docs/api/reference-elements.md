...
[`frame()`](frame.md) creates an independent `Frame` with `.origin` and
`.relate()`. It has no finite geometry. Model and exposed frame references
provide the `FrameAnchor` contract rather than independent construction methods.
