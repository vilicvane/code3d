...
For complete signatures and limits, see [group](api/group.md),
[expose](api/expose.md), [relate](api/relate.md), [on](api/on.md),
[align](api/align.md), [offset](api/offset.md), [rotate](api/rotate.md),
[pivot](api/pivot.md), [pivotVertex](api/pivot-vertex.md),
[pivotPoint](api/pivot-point.md), [axisEdge](api/axis-edge.md),
[axisLine](api/axis-line.md) and [coupleRotation](api/couple-rotation.md).

## Touch a directional bound

`part.relate(() => on(base.up))` places the part above the base by translating its matching
...
Import `on` and `align` from `@code3d/core`. `on(targetBound)` uses the whole
current `relate` self. Use `on(sourceElement, targetBound)` to select a vertex,
edge, surface, or named reference explicitly. Only that selected
...
```ts
const positioned = part.relate(self => align(self.origin, base.origin));
const mounted = part.relate(self => align(self.frame, base.frame));
```
...
For fixed-axis transmission, use
`coupleRotation(driver, {ratio: -0.5})` to couple
the current model to the driver using each model’s own axis, leaving position
free. Full turns survive the ratio, and ordinary
frame attachments carry the driven angle to other parts. See
[rotation coupling](api.md#rotation-coupling) for the supported driving chain and
[Gears](../../gears/docs/api.md#drive-through-connected-parts) for automatic tooth
ratios and engagement phases.

## Choose a rotation pivot or axis

```ts
import {on, pivot} from '@code3d/core';

const placed = part.relate(() => [
  on(base.up),
...
constraint must involve that value, even when written as `on(base, self.up)`.
...
const leftSide = side;
const rightSide = side.relate(() => [on(leftSide.right), offset(160, 0, 0)]);
...
`on` and `align` have inspectors for the call and each explicit source or target
argument. Selecting a reference emphasizes that reference. Selecting the function
name or placing the caret immediately before its opening parenthesis emphasizes
the current `relate` self and its participating reference, keeping the other side
visible as context. This follows self’s identity even when self is the second
argument. `on(target)` also shows self’s complete contact extent. In
`on(base, self.up)`, selecting `self.up` therefore focuses self's bound. Independent
...
The start of a constraint call, such as `|align(self.axis, base.axis)`, keeps
the enclosing `relate` self as the spatial tool owner. Plain rotate and pivot selectors match point rotation; axis selectors match
