...
for (const operation of [
  union,
  intersect,
  inputs => cut(inputs[0], inputs.slice(1)),
]) {
  test(`${operation.name || 'cut'} preserves boundary contributions from both inputs`, () => {
    ...
    try {
      const topology = result.geometry.value.topology;
      for (const kind of ['surfaces', 'edges']) {
        assert.ok(
...
test('n-ary Boolean input paths exclude internal steps and cached prefixes remain reusable', () => {
  ...
  try {
    ...
    for (const [kind, count] of [
      ...
      const ids = new TopologyIdSet(result.geometry.value.topology[kind].ids);
      ...
      const nestedIds = new TopologyIdSet(
        nested.geometry.value.topology[kind].ids,
      );
    ...
    assert.deepEqual(
      posed.geometry.value.topology,
      nested.geometry.value.topology,
    );
    assertMeshIds(posed);
  } finally {
...
test('splits and merges retire ambiguous sources instead of choosing an input', () => {
  ...
  try {
    const {shape, topology} = stock.geometry.value;
    ...
    for (const kind of ['surfaces', 'edges', 'vertices']) {
      assert.ok(
        merged.geometry.value.topology[kind].ids.every(
          id => typeof id === 'number',
        ),
      );
    }
...
test('single-input modifications prefix inherited paths and accept path selections', () => {
  ...
  try {
    ...
    assert.ok(chamfered.geometry.value.topology.edges.ids.includes(1));
...
function assertMeshIds(model) {
  ...
  for (const [kind, ids] of [
    ...
    const {shape, topology} = model.geometry.value;
    for (const id of ids) {
      assert.ok(
        withTopologyShape(
          shape,
...
test('shell accepts loft cap paths and adds one operation level to retained boundaries', () => {
  ...
  try {
    assert.equal(hollow.geometry.id, repeated.geometry.id);
    assert.deepEqual(
    ...
    assert.ok(
      closed.geometry.value.topology.surfaces.ids.some(
        id => typeof id === 'number',
      ),
    );
