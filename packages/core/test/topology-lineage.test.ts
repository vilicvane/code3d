...
for (const sides of [4, 5]) {
  test(`loft retains both caps and all section edges/vertices for ${sides} sides`, () => {
    ...
    const end = regularPolygon(3, sides).relate(p => on(p, location.up));
...
test('mixed loft sections retire split edges and do not invent middle cap faces', () => {
  ...
  const middle = regularPolygon(4, 6).relate(p => on(p, middleLocation.up));
  const end = rectangle(6, 4).relate(p => on(p, endLocation.up));
...
test('shell accepts loft cap paths and retains their original namespace', () => {
  ...
  const end = rectangle(18, 12).relate(p => on(p, point([0, 32, 0]).up));
...
test('ruled loft retains intermediate section edges without creating a cap', () => {
  ...
  const middle = rectangle(14, 10).relate(p => on(p, point([0, 10, 0]).up));
  const end = rectangle(10, 6).relate(p => on(p, point([0, 20, 0]).up));
