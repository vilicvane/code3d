...
an independent transformation after the joint constraint segment. Numeric edits
inside a constraint chain still solve that segment together with its siblings.
...
own result. `pivot`, `pivotVertex`, `pivotPoint`, `aroundEdge`, and `aroundLine` show the preceding pose with
the selected rotation reference. Later calls do not move this preview or its
handles. Sibling constraints in the same continuous segment remain jointly solved;
previous segments provide the incoming pose, and later segments are excluded from
this preview. Only the selected chain is replaced by its selected prefix. If that
prefix conflicts with a sibling constraint, the App reports the preview error and
keeps the valid final model available.
...
Selecting self shows the complete placement segment. Its position and rotation tools edit
the nearest following `offset` and `rotate` respectively; if that operation is
absent, they append one at the end of that segment. In a multi-constraint array,
...
Choose another tool from the toolbar to insert its operation immediately after the selected call,
before later operations. For independent transformations this inserts a new array
