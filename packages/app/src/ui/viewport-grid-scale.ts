import {formatGridStep, majorGridCells} from '../grid-scale';
...
export class ViewportGridScale {
  ...
  constructor(container: HTMLElement, readStep: () => number | undefined) {
    ...
    // Highlight one minor cell within the complete major interval.
    const cellEnd = 2.5 + 55 / majorGridCells;
    for (const [className, d] of [
      ['viewport-grid-scale-remainder', `M${cellEnd} 6.5H57.5M57.5 2.5V10.5`],
      [
        'viewport-grid-scale-cell',
        `M2.5 2.5V10.5M2.5 6.5H${cellEnd}M${cellEnd} 2.5V10.5`,
      ],
    ]) {
  ...
  private render(step: number | undefined): void {
    ...
    const description = `Grid: ${text} per highlighted cell · ${majorGridCells} cells per major interval`;
