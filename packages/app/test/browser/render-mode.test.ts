...
test(
  ...
  async t => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const source = `import {on, offset, box, group, rectangle, line, point} from '@code3d/core';
const base = box(30, 4, 20).material('#48a');
const part = box(10, 12, 8).material('#f008').relate(self => [on(self, /* target */ base.up), offset(0, 2, 0)]);
