...
test('watches before the first package snapshot and invalidates newly created published files', async () => {
  ...
  try {
    ...
    plugin.configureServer({
      watcher: {
        add() {
          ...
          plugin.hotUpdate.call(updates, {file: path.join(core, 'index.js')});
        },
      },
      middlewares: {use() {}},
    });
    const first = await plugin.load('\0virtual:code3d-browser-packages');
    ...
    plugin.hotUpdate.call(updates, {file: extra});
    assert.equal(reloads, before + 1);
    const second = await plugin.load('\0virtual:code3d-browser-packages');
    ...
    plugin.hotUpdate.call(updates, {file: extra});
    assert.doesNotMatch(
      await plugin.load('\0virtual:code3d-browser-packages'),
      /extra\.js/,
    );
  } finally {
    await server.close();
